...
function planetling:new(mass, radius, xpos, ypos, xvel, yvel)
  local object = {
    mass = mass;
    radius = radius;
    xpos = xpos;
...
function planetling:update(dt)
  ...
  grav = (gravity * self.mass) / (math.pow((math.abs(self.xpos) + math.abs(planets[self.grabbed].xpos)), 2) + math.pow((math.abs(self.ypos) + math.abs(planets[self.grabbed].ypos)), 2)) * 1.1;
...
function planetling:setClosestPlanet()
  ...
  min = math.min(distFrom1, distFrom2)
...
function planetling:draw()
  --love.graphics.setColor(love.HSL(280, (self.xvel + self.yvel), 100))
  love.graphics.setColor(100/255, 220/255, 255/255);
